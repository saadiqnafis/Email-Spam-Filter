...
class NaiveBayes:
    ...
    def train(self, data, y):
        ...
        Ncw = np.zeros((self.num_classes, num_features))
        N = data.shape[0]
        Nc = []
        ...
        self.class_priors = (np.array(Nc))/N
        for i in range(self.num_classes):
            
            indices = np.where(y==i)
            
            for j in range(num_features):
                Ncw[i,j] = np.sum(data[indices, j])

        sum = np.sum(Ncw, axis = 1)
        sum = sum.reshape(sum.shape[0],1)
        self.class_likelihoods = (Ncw+1)/(sum+num_features)
        # print(self.class_likelihoods.shape)
        return self.class_priors, self.class_likelihoods
    ...
    def predict(self, data):
        ...
        for i in range (data.shape[0]):
            row_data = data[i,:]
            row_data = row_data.reshape(row_data.shape[0], 1)
            # print(self.get_likelihoods().shape, row_data.shape)
            log_post = row_data.T@self.get_likelihoods().T+self.get_priors()
            predicted_class.append(np.argmax(log_post))
    ...
    def accuracy(self, y, y_pred):
        ...
        '''
        # pass
        comparison = np.count_nonzero(y == y_pred) / y.shape[0]
        # print(y.shape[0])
        return comparison
    ...
    def confusion_matrix(self, y, y_pred):
        ...
        for i in range(self.num_classes):
            for j in range(self.num_classes):
                count = np.count_nonzero((y_pred == j) & (y==i))
                matrix[i,j] = count
                # print(matrix)
        return matrix
