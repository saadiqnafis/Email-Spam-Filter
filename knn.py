...
class KNN:
    ...
    def predict(self, data, k):
        ...
        for i in range(data.shape[0]):
            ...
            # print(sample)
            distance = np.sqrt(np.sum(np.square(sample - self.exemplars), axis=1))
            distance_k = np.argsort(distance)
            distance_k = distance_k[:k]
            taught_classes = self.classes[distance_k]
            mode, count = stats.mode(taught_classes)
            # print(mode)
            list.append(mode)
        list = np.array(list)
        list = list.reshape((list.shape[0],))
        # print(list.shape)
        return list
        # mode, count = stats.mode(list)
        # print(mode.shape)
    ...
    def plot_predictions(self, k, n_sample_pts):
        ...
        '''
        pass
        sample_points = np.linspace(-40,40, n_sample_pts)
        ...
        predicted = predicted.reshape(n_sample_pts,n_sample_pts)
       
        cmap = ListedColormap(cartocolors.qualitative.Safe_4.mpl_colors)
        plt.pcolormesh(sample_points, sample_points, predicted, cmap=cmap, shading='auto')
        plt.colorbar()
        # print(x.shape)
        # print(x)
        # print(y)
